...
def process_video(video_path: Path, similarity_threshold: float, semaphore: Semaphore):
    with semaphore:
        try:
            ...
            if not cap.isOpened():
                return f"❌ {video_path.name}: Unable to open video."

            temp_dir = video_path.parent / (video_path.stem + "_scenes_temp")
            os.makedirs(temp_dir, exist_ok=True)

            existing_images = sorted(temp_dir.glob("frame_*.jpg"))
            if existing_images:
                last_image = max(existing_images, key=lambda p: extract_frame_number_from_filename(p.name))
                resume_from_frame = extract_frame_number_from_filename(last_image.name) + frame_interval
            else:
                resume_from_frame = 0
            ...
            last_hashes = None
            scene_count = len(existing_images)
            frame_index = resume_from_frame

            total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
            progress_bar = trange(frame_index, total_frames, frame_interval, desc=video_path.name[:30], leave=False)
                ...
                if last_hashes is not None:
                    ...
                    if final_similarity < similarity_threshold:
                        if last_scene_image is not None:
                            scene_filename = temp_dir / f"frame_{frame_index:06d}.jpg"
            ...
            image_files = sorted(temp_dir.glob("frame_*.jpg"), key=lambda p: extract_frame_number_from_filename(p.name))
            if image_files:
...
def main():
    ...
    videos_to_process = [v for v in videos if not v.with_suffix(".pdf").exists()]

    if not videos_to_process:
        print("No videos left to process.")
        return

    print(f"\nFound {len(videos_to_process)} video(s) to process. Using {thread_limit} worker(s)...\n")
    ...
    with ThreadPoolExecutor(max_workers=thread_limit) as executor:
        futures = {
            executor.submit(process_video, video, similarity_threshold, semaphore): video
